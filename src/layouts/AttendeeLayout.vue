  ...
  <div class="flex min-h-screen bg-background">
    ...
    <aside :class="['fixed lg:relative w-64 h-screen bg-card border-r border-border flex flex-col transition-all z-40', !mobileMenuOpen && '-translate-x-full lg:translate-x-0']">
      ...
      <nav class="flex-1 overflow-y-auto p-4 space-y-2">
        <nav-item to="/dashboard" icon="🏠">Dashboard</nav-item>
        <nav-item to="/dashboard/events" icon="🎉">Browse Events</nav-item>
        <nav-item to="/dashboard/favorites" icon="❤️">Favorites</nav-item>
        <nav-item to="/dashboard/tickets" icon="🎫">My Tickets</nav-item>
      </nav>
    </aside>
    ...
    <main class="flex-1 flex flex-col w-full">
      ...
      <header class="border-b border-border bg-card px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between sticky top-0 z-30">
        ...
        <div class="flex items-center gap-3">
          <router-link to="/dashboard/notifications" class="p-2 hover:bg-muted rounded-lg relative">
            <span class="text-xl">🔔</span>
            <span class="absolute top-1 right-1 w-2 h-2 bg-destructive rounded-full"></span>
          </router-link>
          <router-link to="/dashboard/settings" class="p-2 hover:bg-muted rounded-lg">
